...
const CartSummary = () => {
  const { cartItems, cartSubtotal, cartTotal, appliedVoucher, setAppliedVoucher } =
    useCart();
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="border border-zinc-200 p-6 bg-white">
        ...
        <button 
          onClick={() => {
            const checkoutItems = cartItems.map(item => ({...item, selected: true}));
            localStorage.setItem("mando_checkout", JSON.stringify(checkoutItems));
            navigate("/checkout");
          }}
          className="w-full bg-black text-white py-4 text-sm font-bold tracking-widest hover:bg-zinc-900 transition-colors uppercase"
        >
